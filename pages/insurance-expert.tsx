...
export default function Chat() {
  ...
  const [assistantName, setAssistantName] = useState('Pojistné podmínky');
  const [assistantModel, setAssistantModel] = useState('gpt-4-1106-preview');
  const [assistantDescription, setAssistantDescription] = useState('Vystupuj jako zkušený operátor a pojišťovacího agent v pojišťovně. Odpovídej na dotazy. Uživateli tykej, je to tvůj kolega. Neposílej mě za pojišťovacím agentem, neobracej mě na pojišťovnu, ani mi nedoporučuj přečíst si pojistné podmínky. Zdroj uváděj jako název souboru. Komunikuj v češtině.');
