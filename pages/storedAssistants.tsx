...
const StoredAssistantsPage: NextPage = () => {
  ...
  const removeChatFile = (fileName: string) => {
    ...
    const startChatAssistant = async (assistantId: string | null, threadId: string | null) => {
        setIsButtonDisabled(true);
        setStartLoading(true);
        if (chatManager) {
            try {
                await chatManager.resumeAssistantConversation(assistantId!, threadId!);
    ...
    return (
        <main className="flex flex-col items-center justify-between pb-40 bg-space-grey-light">
            <LinkBar />
            <AssistantList startChatAssistant={startChatAssistant} />
